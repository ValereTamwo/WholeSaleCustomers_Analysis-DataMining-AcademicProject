...
tabsetPanel(
  ...
  tabPanel(class="text-[arial] ml-[5vw]" ,"Data",
           tabsetPanel(  tabPanel('Original Data',
                                  ...
                                  DTOutput('dataclean'),downloadButton('save','Download')
  ...
  ,
  tabPanel('unsuperviced learning',
...
cleaning = function(){
  ...
  output$test = renderDataTable({
   ...
   #--------------------------Resumer des Donnees ---------------
     ...
     ##sauvegarde du jeu propre
     output$save=downloadHandler(filename = function(){
       paste('clean data_',Sys.Date(),'.csv',sep = '') #Sys.Date() met la date du jour
     },
     ...
     output$dat2=renderDataTable({input$clu
       isolate({data_norm})})
